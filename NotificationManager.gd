...
func _on_notification_opened(notification_id: int):
	_debug_log("📬 Notification opened: ID=%d" % notification_id)

func _on_notification_dismissed(notification_id: int):
	_debug_log("🗑️ Notification dismissed: ID=%d" % notification_id)
...
# Add this variable at the top of your script to track active touches
var active_touches = {}

func _input(event):
	"""Handle 3-finger tap for debug mode toggle"""
	if event is InputEventScreenTouch:
		if event.pressed:
			# Finger pressed - add to active touches
			active_touches[event.index] = true
			
			# Check if exactly 3 fingers are touching
			...
			# Finger released - remove from active touches
